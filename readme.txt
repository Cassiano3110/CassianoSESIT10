...
Copyright 2004-2016 by DesignSoft-Texas Instruments
...
applications for TI’s products.
...
New features in Tina-TI 9.3.200

 - MOS and BSIM3 model equation fixes
 
New features in Tina-TI 9.3.180

 - Undo/Redo in schematic symbol editor
 - case insensitive jumper connections
 - spice syntax update: 
     dot is allowed in subckt names, 
     ASINH, ACOSH, ATANH functions are supported
     mfg parameter can be used in .MODEL 
 - transient analysis with "Zero initial values" if "Calculate operating point" fails
 - name of 1st parameter is shown for components when the 1st parameter is not a typical parameter
 - txt exetension is allowed in New macro wizard spice file import mode
 
New features in Tina-TI 9.3.150

 - New Windows 10 compatible installer
 - 260+ new models (+30 added in this edtition)
 - spice syntax update: 
    empty and double PARAMS: can be used in subcircuit calls; 
    .IC command can be used in subcircuits, dot is allowed in model names
 - hotkeys added for analysis menus
 - error message shown in Macro Wizard if input file is locked
 - ability to search for a signal in postprocessor
 - while importing a model  'Auto generate shape' is the default
					
New features in Tina-TI 9.3

- New Fourier THD view: D * cos(kwt + fi)
- Extended help for Fourier analysis
- Import/export 3 column AC signal file (Freq, Gain in dB, Phase)
- Improved avarage function calculation
- Display MEG instead of M
- New help files specific to TINA-TI
- Encrypted VHDL macros
- Programmable digital data generators: Parallel and SPI
...
Program Version: 9.3.200.277; Tina-TI
